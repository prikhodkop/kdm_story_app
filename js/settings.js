...
function addSettings(settings) {
  $('#container').append($('<div>',{id:'settings-window-background'}));
  ...
  $('#container').append($('<div>',{id:'settings-window-back0'}));
  $('#settings-window-back0').append($('<img>',{id:'settings-window-back-img', src:'images/reference/reference_back.png'}));
  $('#settings-window-back0').append($('<div>',{id:'settings-window-back'}));
  ...
  $('#settings-window-background').on('click', function(){
    ...
    $('#settings-window-background').fadeOut(500);
    $('#settings').removeClass('active');
    setTimeout(function() {setSettings(settings);}, 600);
  });
...
function createTable(schema, defaults=undefined, level=0, group='') {
  ...
  console.log(defaults)
  let table = $('<table>',{class:'settings', level:level})
  let tbody = $('<tbody>',{class:'settings', level:level})
  table.append(tbody)

  for (var key in schema) {
    console.log('Key:')
    console.log(key)
    let tr = $('<tr>',{class:'settings', level:level})
    tbody.append(tr)
    if (schema[key]['type'] == 'option') {
      let td1 = $('<td>',{class:'settings', id:'title', level:level})
      td1.text(schema[key]['title'])
      tr.append(td1)
      let td2 = $('<td>',{class:'settings', id:'select', level:level})

      let sel = $('<select>',{class:'settings', level:level, value:key, group:group}).appendTo(td2);
      $(schema[key]['enum']).each(function() {
        sel.append($('<option>',{class:'settings', id:level, parent:key, group:group}).attr('value',this).text(this));
      });
    ...
    if (schema[key]['type'] == 'group') {
      let td1 = $('<td>',{class:'settings', id:'group', level:level})
      td1.text(schema[key]['title'])
      tr.append(td1)

      let tr2 = $('<tr>',{class:'settings', level:level})
      tbody.append(tr2)

      let td2 = $('<td>',{class:'settings', id:'table', level:level, colspan:2, group:schema[key]})
