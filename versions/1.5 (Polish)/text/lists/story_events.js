var texts = {
  "index": {
   'label': "Intro",},
  "hunt": {
   'label': "Łowy",},
  "settlement": {
   'label': "Osada",},
  "age": {
   'label': "Wiek",},
  "armored strangers": {
   'label': "Nieznajomi w zbrojach",},
  "birth of a savior": {
   'label': "Narodziny Zbawcy",},
  "blackout": {
   'label': "Wyciemnienie",},
  "bold": {
   'label': "Śmiałość",},
  "bone witch": {
   'label': "Wiedźma kości",},
  "cooking": {
   'label': "Gotowanie",},
  "crush and devour": {
   'label': "Zmiażdżenie i pożarcie",},
  "endless screams": {
   'label': "Przeciągłe wycie",},
  "game over": {
   'label': "Koniec gry",},
  "hammer and nail": {
   'label': "Młot i gwóźdź",},
  "hands of heat": {
   'label': "Ręce ciepła",},
  "herb gathering": {
   'label': "Zbieranie ziół",},
  "hooded knight": {
   'label': "Zakapturzony rycerz",},
  "insight": {
   'label': "Wizja",},
  "intimacy": {
   'label': "Współżycie",},
  "kings curse": {
   'label': "Królewska klątwa",},
  "kings step": {
   'label': "Królewskie stąpnięcie",},
  "lantern research": {
   'label': "Badania latarni",},
  "legendary lungs": {
   'label': "Legendarne płuca",},
  "legendary monsters": {
   'label': "Legendarne potwory",},
  "mineral gathering": {
   'label': "Zbieranie minerałów",},
  "overwhelming darkness": {
   'label': "Przytłaczająca ciemność",},
  "oxidation": {
   'label': "Oksydacja",},
  "phoenix feather": {
   'label': "Pióro Feniksa",},
  "principle conviction": {
   'label': "Doktryna: przekonania",},
  "principle death": {
   'label': "Doktryna: śmierć",},
  "principle new life": {
   'label': "Doktryna: nowe życie",},
  "principle society": {
   'label': "Doktryna: społeczna",},
  "first story": {
   'label': "Pierwsza przygoda",},
  "regal visit": {
   'label': "Królewska wizytacja",},
  "returning survivors": {
   'label': "Powracający Ocalali",},
  "run away": {
   'label': "Ucieczka",},
  "see the truth": {
   'label': "Ujrzenie prawdy",},
  "showdown butcher": {
   'label': "Starcie: Rzeźnik",},
  "showdown gold smoke knight": {
   'label': "Starcie: Złoty Kondotier",},
  "showdown kings man": {
   'label': "Starcie: Wysłannik",},
  "showdown phoenix": {
   'label': "Starcie: Feniks",},
  "showdown screaming antelope": {
   'label': "Starcie: Wrzeszcząca Antylopa",},
  "showdown the hand": {
   'label': "Starcie: Namiestnik",},
  "showdown watcher": {
   'label': "Starcie: Obserwator",},
  "showdown white lion": {
   'label': "Starcie: Biały Lew",},
  "watched": {
   'label': "Obserwowani",},
  "white secret": {
   'label': "Biały sekret",},
  "white speaker": {
   'label': "Biała mówczyni",},
  "zero presence": {
   'label': "Nieuchwytność",},
  
   "fetid grotto": {
   'label': "Cuchnąca pieczara",},
  "final march": {
   'label': "Ostatni marsz",},
  "the approaching storm": {
   'label': "Nadchodząca burza",},
  "melting horror": {
   'label': "Rozpuszczająca groza",},
  "showdown gorm": {
   'label': "Starcie: Gorm",},
   
  "black harvest": {
   'label': "Black Harvest",},
  "rumbling in the dark": {
   'label': "Tumbling in the Dark",},
  "secret meeting": {
   'label': "Secret Meeting",},
  "spelunking of death": {
   'label': "Spelunking of Death",},
  "underground sow": {
   'label': "Underground Sow",},
  "showdown dung beetle knight": {
   'label': "Showdown: Dung Beetle Knight",},
  "an uninvited guest": {
   'label': "An Uninvited Guest",},
  "finale": {
   'label': "Finale",},
  "intermission": {
   'label': "Intermission",},
  "places, everyone!": {
   'label': "Places, Everyone!",},
  "showdown lion knight": {
   'label': "Showdown: Lion Knight",},
  "strange caravan": {
   'label': "Strange Caravan",},
  "awake": {
   'label': "Awake",},
  "death of the dragon king": {
   'label': "Death of the Dragon King",},
  "faces in the sky": {
   'label': "Faces in the Sky",},
  "foundlings": {
   'label': "Foundlings",},
  "glowing crater": {
   'label': "Glowing Crater",},
  "meltdown": {
   'label': "Meltdown",},
  "midnight's children": {
   'label': "Midnight's Children",},
  "showdown dragon king": {
   'label': "Showdown: Dragon King",},
  "showdown the tyrant": {
   'label': "Showdown: the Tyrant",},
  "the tomb": {
   'label': "The Tomb",},
  "unveil the sky": {
   'label': "Unveil the Sky",},
  "bleeding heart": {
   'label': "Bleeding Heart",},
  "death pit": {
   'label': "Death Pit",},
  "lottery": {
   'label': "Lottery",},
  "showdown manhunter": {
   'label': "Showdown: Manhunter",},
  "sonorous rest": {
   'label': "Sonorous Rest",},
  "the hanged man": {
   'label': "The Hanged Man",},
  "tools of war": {
   'label': "Tools of War",},
  "dark place": {
   'label': "Ciemne miejsce",},
  "forgotten fear": {
   'label': "Zapomniany strach",},
  "light-forging": {
   'label': "Kucie światłem",},
  "showdown slenderman": {
   'label': "Starcie: Smukłak",},
  "it's already here": {
   'label': "Już tu jest",},
  "a gracious host": {
   'label': "A Gracious Host",},
  "death reading": {
   'label': "Death Reading",},
  "necropolis": {
   'label': "Necropolis",},
  "showdown lion god": {
   'label': "Showdown: Lion God",},
  "the knowledge worm": {
   'label': "The Knowledge Worm",},
  "the silver city": {
   'label': "The Silver City",},
  "birth of color": {
   'label': "Birth of Color",},
  "conquer your shadow": {
   'label': "Conquer Your Shadow",},
  "edged tonometry": {
   'label': "Edged Tonometry",},
  "final gift": {
   'label': "Final Gift",},
  "showdown sunstalker": {
   'label': "Showdown Sunstalker",},
  "sky fishing": {
   'label': "Sky Fishing",},
  "sun dipping": {
   'label': "Sun Dipping",},
  "the great devourer": {
   'label': "The Great Devourer",},
  "the great sky gift": {
   'label': "The Great Sky Gift",},
  "the pool and the sun": {
   'label': "The Pool and the Sun",},
  "umbilical symbiosis": {
   'label': "Umbilical Symbiosis",},
  "warriors of the sun": {
   'label': "Warriors of the Sun",},
  "promise under the sun": {
   'label': "Promise Under the Sun"},
}

module.exports = {
 texts,
}
